...
from kafkalib import Kafka, Signal, SignalEvent, Timeframe, Topics
...
class StrategyBuilder:
    def __init__(
        ...
        # Built here rather than at module scope: importing strategylib must not
        # open a database pool or reach for a Kafka broker.
        self.ds = DataStore()
        self.kafka = Kafka()
        self.kafka_app = self.kafka.get_app()
        ...
        strategyConfig = self.remote_store.get_strategy(strategy_name=name)

        if strategyConfig is None:
            self.remote_store.create_strategy(
                Strategy(
                    name=name,
                    description="",
                    run_tf=run_tf,
        ...
        for tick in self.config.tickers:
            self.ds.add_to_priority(tick)

    def __del__(self):
        # Remove tickers from priority when strategy is deleted
        for tick in self.config.tickers:
            self.ds.remove_from_priority(tick)
    ...
    def run(self):
        datafeed_topic = self.kafka.get_feed_topic(self.config.run_tf)
