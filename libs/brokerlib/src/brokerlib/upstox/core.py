...
from .auth import authorize, get_client_credentials
...
class UpstoxBroker:
    def __init__(self, strategy: str):
        ...
        # Built per instance rather than at module scope: importing brokerlib
        # must not require broker credentials or a database.
    ...
    def _authorize(self):
        if not self.creds.get_credential("upstox.token"):
            authorize()

        token = self.creds.get_credential("upstox.token")
        last_fetched = self.creds.get_credential("upstox.last_fetched")
        ...
        self.token = self.creds.get_credential("upstox.token")
    ...
    def _get_instrument_key(self, ticker: str):
        ...
        data = self.ds.get_ticker(ticker)
    ...
    def order_send(
        ...
        try:
            ticker = self.ds.get_ticker(ticker)
