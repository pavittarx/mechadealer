...
def calculate_units_from_amount(
    ...
    """Units issued for `amount` invested, at the strategy's current unit price.

    A strategy is created with units=0 and capital=0, so the first investment
    seeds the unit price at 1.0. This previously raised "Strategy units cannot
    be zero", which made it impossible for anyone to be the first investor, and
    divided by strategy_capital while only guarding strategy_units.
    """
    if strategy_units == 0 or strategy_capital == 0:
        return round(amount, 2)

    # Round once, at the end. Rounding the unit price to 2dp first threw away
    # most of the precision whenever the price was small.
    return round(amount * (strategy_units / strategy_capital), 2)


def calculate_amount_from_units(
    strategy_units: float, strategy_capital: float, units: float
) -> float:
    """Value of `units`, the inverse of calculate_units_from_amount."""
    if strategy_units == 0:
        raise ValueError("Strategy units cannot be zero")

    return round(units * (strategy_capital / strategy_units), 2)
...
class Store:
    ...
    def _get_conn(self):
        if self._conn:
            return self._conn
        return get_engine().begin()  # Use global engine directly
