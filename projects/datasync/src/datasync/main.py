...
import pandas as pd
import pytz
from coreutils import CronTrigger, Logger, scheduler
...
def sync_instruments():
    ...
    df: pd.DataFrame = pd.concat([nse, bse], ignore_index=True)
    ...
    # Filter out existing records. Bound to a new name rather than reassigning
    # `df`, which made its inferred type recursive under pandas 3 stubs.
    new_rows = df[~df["query_key"].isin(list(existing_keys))]
    ...
    with db.get_connection() as conn:
        ...
        values = [
            tuple(row)
            for row in cast(pd.DataFrame, new_rows[insert_columns]).to_records(
                index=False
            )
        ]
...
def trigger_kafka_events(tick: dict[str, str], data):
    ...
    feed_1M = k.get_topic(Topics.FEED_1M, value_serializer="json")
    row = data.iloc[0]
        ...
        if data_1M is not None:
            ...
            for tf in timeframe:
                if int(time_diff % tf_multiplier[tf]) == 0:
                    topic_name = Topics["FEED_" + tf].value
