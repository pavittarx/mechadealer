...
from kafkalib import Kafka, SignalEvent, Topics
from storelib import Order, Store
...
def handle_stop_orders(order_id: int):
    ...
    if not order:
        logger.error("Order not found in store: %s", order_id)
        return
    ...
    for stop_order in stop_orders:
        ...
        if not stop_order_details:
            logger.error("Order details not found in broker: %s", stop_order.broker_id)
            continue
...
def main():
    ...
    try:
        ...
        with app.get_consumer() as consumer:
            consumer.subscribe([Topics.SIGNALS.value])
